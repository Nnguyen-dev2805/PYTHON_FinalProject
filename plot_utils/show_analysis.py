...
def show_top(df, column, metric, title): 
    ...
    if column not in df.columns or metric not in df.columns:
        messagebox.showerror("Lỗi", "Cột hoặc chỉ số không hợp lệ!")
        return
...
def draw_analysis(df, column, metric, title):
    ...
    # Vẽ biểu đồ
    plt.figure(figsize=(30, 17))  # Set figure size
    sns.barplot(x=top[metric], y=top[column])
    plt.title(title, size=30)
    if metric == "popularity":
        plt.xlabel("Độ phổ biến", size=20)
    elif metric == "budget_adj":
        plt.xlabel("Ngân sách", size=20)
    else:
        plt.xlabel("Doanh thu", size=20)
    plt.ylabel("Tên phim", size=20)
